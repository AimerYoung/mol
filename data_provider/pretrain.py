...
from data_provider.pubchem import PubChemDataset
...
class TrainCollater(object):
    ...
    def __call__(self, batch):
        data_list, text_list, words_list, text_edge_index_list = zip(*batch)
        ...
        return graph_batch, text_batch.input_ids, text_batch.attention_mask, list(words_list), list(text_edge_index_list)
...
class PretrainDM(LightningDataModule):
    def __init__(
        ...
        if False:
            ...
            self.train_dataset = PubChemDataset(root+'train.pt', text_max_len)
            self.val_dataset = PubChemDataset(root + 'valid.pt', text_max_len)
            self.val_dataset_match = PubChemDataset(root + 'valid.pt', text_max_len).shuffle()
            self.test_dataset_match = PubChemDataset(root + 'test.pt', text_max_len).shuffle()
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--root', type=str, default='data/PubChem324kSP')
