...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # print("GIN edge_index", edge_index)
        # print("GIN x", x)
        # print("num nodes:", x.size(0))
        # print("GIN edge_index", max(edge_index[0]), max(edge_index[1]))
...
class GATConv(MessagePassing):
    def __init__(self, emb_dim, heads=2, negative_slope=0.2, aggr = "add"):
        super(GATConv, self).__init__(node_dim=-3)
        self.aggr = aggr
...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        # print(x[:,0].dtype)
        # print(x[:,0])
        # print(self.x_embedding1(x[:,0]).shape)
        # print(self.x_embedding2(x[:,1]).shape)
        # breakpoint()
...
class TextGNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, heads, JK = "last", drop_ratio = 0):
        ...
        # file_path = '/home/zhongcl/M_T_Data/kv_data/all_nodes_features.pt'
        # node_features = torch.load(file_path)

        # # 将加载的特征转化为嵌入层的参数
        # self.x_embedding1 = torch.nn.Embedding.from_pretrained(node_features['node_features'], freeze=False)  # Node_num * 728
        # self.x_embedding2 = torch.nn.Embedding.from_pretrained(node_features['pos_features'], freeze=False)   # rel_num * 728


        self.x_embedding_rels = torch.nn.Embedding(rels_type, emb_dim)  # 47*728
        torch.nn.init.xavier_uniform_(self.x_embedding_rels.weight.data)

        ###List of MLPs
        self.gnns = torch.nn.ModuleList()
        for layer in range(num_layer):
            self.gnns.append(GATConv(emb_dim, heads=heads))
    ...
    def forward(self, *argv):
        ...
        # 该参数是一个 Data 对象
            ...
            words, sentence, edge_index, rels, batch = data.words, data.sentence, data.edge_index, data.rels, data.batch
        else:
            raise ValueError("unmatched number of arguments.")
        
        x = self.words_to_embeddings(words, sentence)
        x = x + self.x_embedding_rels(rels)
        # x_unpacked, lengths = pad_packed_sequence(x, batch_first=True)
        # print(x_unpacked)
        # print(x_unpacked.shape)
        # print(self.x_embedding_rels(rels))
        # print(self.x_embedding_rels(rels).shape)
        # x = x_unpacked + self.x_embedding_rels(rels)
        # x = self.x_embedding1(x[:,0]) + self.x_embedding2(x[:,1])
        # edge_index = self.get_edge_index(arcs)
    ...
    def words_to_embeddings(self, words, sentences):
        all_embeddings = []
        for word, sentence in zip(words, sentences):
            words_offsets = self.get_offsets(word,sentence)
            ...
            all_embeddings.append(torch.stack(word_embeddings))  # 使用 stack 将单词嵌入转化为张量

        # 将所有的单词嵌入合并为一个最终的张量
        final_embeddings = torch.cat(all_embeddings, dim=0)  # 合并所有单词的嵌入
