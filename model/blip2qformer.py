...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        self.text_encoder = self.init_text_encoder(num_layers=3, hidden_dim=768, device=device) 
    ...
    def forward(self, batch):
        ...
        ###============== Image-text Contrastive ===================###
        graph, input_ids, mask, word, text_edge_index = batch
        batch_node, batch_mask = self.graph_encoder(graph)
        ...
        graph_feats = self.graph_proj(query_output.last_hidden_state) # shape = [B, num_q, D]
        text_feats = self.text_encoder(word, text_edge_index)
        # text_output = self.Qformer.bert(text, attention_mask=mask, return_dict=True) # shape = [B, n_max, D]
        # text_feats = self.text_proj(text_output.last_hidden_state[:, 0, :])
        ...
        if self.gtm:
            ...
            query_tokens_itm = self.query_tokens.expand(text_ids_all.shape[0], -1, -1)
            query_atts_itm = torch.ones(query_tokens_itm.size()[:-1], dtype=torch.long, device=input_ids.device)
            attention_mask_all = torch.cat([query_atts_itm, text_atts_all], dim=1)
        ...
        if self.lm:
            ...
            query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long, device=input_ids.device)
