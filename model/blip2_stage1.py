...
class Blip2Stage1(pl.LightningModule):
    def __init__(self, args):
        ...
        self.blip2qformer = Blip2Qformer(args.gtm, args.lm, args.bert_name, args.temperature, args.gin_num_layers, args.gin_hidden_dim, args.drop_ratio, args.tune_gnn, args.num_query_token, args.cross_attention_freq, args.projection_dim, args.devices)
...
def eval_retrieval_inbatch_with_rerank(model, dataloader, device=None):
    ...
    for batch in tqdm(dataloader):
        aug, text, text_mask, word, text_edge_index = batch
        # aug, text, text_mask = batch
        text_total.append(text)
        text_mask_total.append(text_mask)
